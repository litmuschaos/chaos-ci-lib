...
func InstallPodNetworkLossEngine(experimentsDetails *types.ExperimentDetails) error {
	...
	if err = pkg.EditKeyValue("/tmp/"+experimentsDetails.ExperimentName+"-ce.yaml", "NETWORK_PACKET_LOSS_PERCENTAGE", "value: '100'", "value: '"+strconv.Itoa(experimentsDetails.NetworkPacketLossPercentage)+"'"); err != nil {
		return errors.Errorf("Fail to Update the engine file, due to %v", err)
	}
