...
var _ = Describe("BDD of running pod-network-corruption experiment", func() {
	...
	Context("Check for pod-network-corruption experiment via SDK", func() {
		...
		It("Should run the pod-network-corruption experiment via SDK", func() {
			...
			experimentRequest, errConstruct := ConstructPodNetworkCorruptionExperimentRequest(&experimentsDetails, experimentID)
...
// ConstructPodNetworkCorruptionExperimentRequest constructs the experiment request by fetching template from external source
func ConstructPodNetworkCorruptionExperimentRequest(details *types.ExperimentDetails, experimentID string) (*models.SaveChaosExperimentRequest, error) {
	...
	enginePath := "https://hub.litmuschaos.io/api/chaos/master?file=charts/generic/pod-network-corruption/engine.yaml"
					...
					if components, compOk := exp["spec"].(map[string]interface{}); compOk {
						if compEnv, envOk := components["components"].(map[string]interface{}); envOk {
							if env, envListOk := compEnv["env"].([]interface{}); envListOk {
								// Set pod-network-corruption specific parameters
								for _, envVar := range env {
									if envMap, isMap := envVar.(map[string]interface{}); isMap {
										...
										} else if envMap["name"] == "NETWORK_PACKET_CORRUPTION_PERCENTAGE" {
											envMap["value"] = fmt.Sprintf("%d", details.NetworkPacketCorruptionPercentage)
