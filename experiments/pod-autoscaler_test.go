...
//BDD for running pod-autoscaler experiment
var _ = Describe("BDD of running pod-autoscaler experiment", func() {

	Context("Check for pod-autoscaler experiment via SDK", func() {
		...
		BeforeEach(func() {
			...
			environment.GetENV(&experimentsDetails, "pod-autoscaler", "pod-autoscaler-engine")
		...
		It("Should run the pod-autoscaler experiment via SDK", func() {
			...
			experimentRequest, errConstruct := ConstructPodAutoscalerExperimentRequest(&experimentsDetails, experimentID)
...
// ConstructPodAutoscalerExperimentRequest constructs the experiment request by fetching template from external source
func ConstructPodAutoscalerExperimentRequest(details *types.ExperimentDetails, experimentID string) (*models.SaveChaosExperimentRequest, error) {
	...
	enginePath := "https://hub.litmuschaos.io/api/chaos/master?file=charts/generic/pod-autoscaler/engine.yaml"
					...
					if components, compOk := exp["spec"].(map[string]interface{}); compOk {
						if compEnv, envOk := components["components"].(map[string]interface{}); envOk {
							if env, envListOk := compEnv["env"].([]interface{}); envListOk {
								// Set pod-autoscaler specific parameters
								for _, envVar := range env {
									if envMap, isMap := envVar.(map[string]interface{}); isMap {
										...
										} else if envMap["name"] == "REPLICAS" {
											envMap["value"] = fmt.Sprintf("%d", details.Replicas)
										}
									}
								}
							}
