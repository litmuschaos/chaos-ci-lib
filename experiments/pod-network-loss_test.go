...
//BDD for running pod-network-loss experiment
var _ = Describe("BDD of running pod-network-loss experiment", func() {

	Context("Check for pod-network-loss experiment via SDK", func() {
		...
		BeforeEach(func() {
			...
			environment.GetENV(&experimentsDetails, "pod-network-loss", "pod-network-loss-engine")
		...
		It("Should run the pod-network-loss experiment via SDK", func() {
			...
			experimentRequest, errConstruct := ConstructPodNetworkLossExperimentRequest(&experimentsDetails, experimentID)
...
// ConstructPodNetworkLossExperimentRequest constructs the experiment request by fetching template from external source
func ConstructPodNetworkLossExperimentRequest(details *types.ExperimentDetails, experimentID string) (*models.SaveChaosExperimentRequest, error) {
	...
	enginePath := "https://hub.litmuschaos.io/api/chaos/master?file=charts/generic/pod-network-loss/engine.yaml"
					...
					if components, compOk := exp["spec"].(map[string]interface{}); compOk {
						if compEnv, envOk := components["components"].(map[string]interface{}); envOk {
							if env, envListOk := compEnv["env"].([]interface{}); envListOk {
								// Set pod-network-loss specific parameters
								for _, envVar := range env {
									if envMap, isMap := envVar.(map[string]interface{}); isMap {
										...
										} else if envMap["name"] == "NETWORK_INTERFACE" {
											envMap["value"] = details.NetworkInterface
										} else if envMap["name"] == "NETWORK_PACKET_LOSS_PERCENTAGE" {
											envMap["value"] = fmt.Sprintf("%d", details.NetworkPacketLossPercentage)
