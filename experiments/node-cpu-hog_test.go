...
//BDD for running node-cpu-hog experiment
var _ = Describe("BDD of running node-cpu-hog experiment", func() {

	Context("Check for node-cpu-hog experiment via SDK", func() {
		...
		BeforeEach(func() {
			...
			environment.GetENV(&experimentsDetails, "node-cpu-hog", "node-cpu-hog-engine")
		...
		It("Should run the node-cpu-hog experiment via SDK", func() {
...
// ConstructNodeCPUHogExperimentRequest constructs the experiment request by fetching template from external source
func ConstructNodeCPUHogExperimentRequest(details *types.ExperimentDetails, experimentID string) (*models.SaveChaosExperimentRequest, error) {
	...
	enginePath := "https://hub.litmuschaos.io/api/chaos/master?file=charts/generic/node-cpu-hog/engine.yaml"
