...
var _ = Describe("BDD of running pod-cpu-hog experiment", func() {
	...
	Context("Check for pod-cpu-hog experiment via SDK", func() {
		...
		BeforeEach(func() {
			...
			environment.GetENV(&experimentsDetails, "pod-cpu-hog", "pod-cpu-hog-engine")
		...
		It("Should run the pod-cpu-hog experiment via SDK", func() {
			...
			experimentRequest, errConstruct := ConstructPodCPUHogExperimentRequest(&experimentsDetails, experimentID)
...
// ConstructPodCPUHogExperimentRequest constructs the experiment request by fetching template from external source
func ConstructPodCPUHogExperimentRequest(details *types.ExperimentDetails, experimentID string) (*models.SaveChaosExperimentRequest, error) {
	...
	enginePath := "https://hub.litmuschaos.io/api/chaos/master?file=charts/generic/pod-cpu-hog/engine.yaml"
							...
							if env, envListOk := compEnv["env"].([]interface{}); envListOk {
								...
								for _, envVar := range env {
									if envMap, isMap := envVar.(map[string]interface{}); isMap {
										if envMap["name"] == "CPU_CORES" {
											envMap["value"] = fmt.Sprintf("%d", details.CPU)
