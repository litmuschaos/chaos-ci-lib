...
//BDD for running pod-memory-hog experiment
var _ = Describe("BDD of running pod-memory-hog experiment", func() {

	Context("Check for pod-memory-hog experiment via SDK", func() {
		...
		BeforeEach(func() {
			...
			environment.GetENV(&experimentsDetails, "pod-memory-hog", "pod-memory-hog-engine")
		...
		It("Should run the pod memory hog experiment via SDK", func() {
            ...
            experimentName := pkg.GenerateUniqueExperimentName("pod-memory-hog")
            experimentsDetails.ExperimentName = experimentName
            experimentID := pkg.GenerateExperimentID()
            experimentRequest, errConstruct := workflow.ConstructPodMemoryHogExperimentRequest(&experimentsDetails, experimentID, experimentName)
...
// ConstructPodMemoryHogExperimentRequest constructs the experiment request by fetching template from external source
func ConstructPodMemoryHogExperimentRequest(details *types.ExperimentDetails, experimentID string) (*models.SaveChaosExperimentRequest, error) {
	...
	enginePath := "https://hub.litmuschaos.io/api/chaos/master?file=charts/generic/pod-memory-hog/engine.yaml"
					...
					if components, compOk := exp["spec"].(map[string]interface{}); compOk {
						if compEnv, envOk := components["components"].(map[string]interface{}); envOk {
							if env, envListOk := compEnv["env"].([]interface{}); envListOk {
								// Set pod-memory-hog specific parameters
								for _, envVar := range env {
									if envMap, isMap := envVar.(map[string]interface{}); isMap {
										...
										} else if envMap["name"] == "MEMORY_CONSUMPTION" {
